 ...
 * -> https://github.com/sreeix/node-redis-raw
 * without any modifications (except code formatting).
 *
 * Using the npm version (node-redis-raw) did not work for me (maybe you can find my error?), but
 * inserting the script into a file and putting a locale require statement worked.
 *
 * sorry sreeix.
 */
...
redis.RedisClient.prototype.on_data = function (data) {
    ...
    if (this.command_queue.peek && this.command_queue.peek().raw) {
        ...
        try_callback(command_obj.callback, data);
    }
    else {
